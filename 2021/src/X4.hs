...
part1 (numsToCall, boards) =
  let go = boardsWithHits

      boardsWithHits :: [Game] =
        map (\b -> (fromList (replicate (length b) False), b)) boards

      doHits :: Int -> Game -> Vector Bool
      doHits called (hits, board) =
        zip [0..] board
      ...
      winningBoard = findWinner $ runGame numsToCall boardsWithHits
...
part2 (numsToCall, boards) =
  let go = boardsWithHits
      ...
      keepTruckn l@(_, b) ((lastCalled, games):xs) =
        let winners = [g | g <- games, winner g]
...
-- main = readFile "4" <&> lines >>> parse >>> part2 >>= print
